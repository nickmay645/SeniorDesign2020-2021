...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    for (x, y, w, h) in faces:
        ...
        # Have it record the time when a face is registered ->> (when x is printed) and set that to a variable if the
        # current time is more than 10 seconds from the originally set value, issue a reset command to set servos to
        # initial position (make that reset state a function)
